...
type lambda =
  ...
  | Lasminline of (string,lambda,Ident.t,lambda) Asm_inline_types.t
...
let make_key e =
  ...
  let rec tr_rec env e =
    ...
    | Lasminline asm ->
        Lasminline (Asm_inline_types.map_exprs (tr_rec env) asm)
    | Lsequence (e1,e2) ->
...
let iter f = function
  ...
  | Lifthenelse(e1, e2, e3) ->
      f e1; f e2; f e3
  | Lasminline asm ->
      Asm_inline_types.iter_exprs f asm
  | Lsequence(e1, e2) ->
      f e1; f e2
  | Lwhile(e1, e2) ->
      f e1; f e2
...
let free_ids get l =
  ...
  let rec free l =
    ...
    | Lasminline asm ->
        Asm_inline_types.iter_outputs
          (fun id -> fv := IdentSet.remove id !fv) asm
...
let subst_lambda s lam =
  ...
  | Lasminline asm -> Lasminline(Asm_inline_types.map_exprs subst asm)
